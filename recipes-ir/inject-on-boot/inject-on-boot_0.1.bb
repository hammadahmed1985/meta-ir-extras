...
LICENSE = "CLOSED"
LIC_FILES_CHKSUM = "file://${WORKDIR}/inject-on-boot-demo.sh;md5=a39556cf1f3d5b2103cc068e8d65edbb"

SRC_URI = "file://inject-on-boot-demo.sh file://11-evdev-excludes.conf file://renew-on-up.sh"
...
do_install () {
    ...
    #
    install -m 0755 ${WORKDIR}/inject-on-boot-demo.sh ${D}${sysconfdir}/init.d/
    ...
    #
    ln -sf ../init.d/inject-on-boot-demo.sh  ${D}${sysconfdir}/rc5.d/S90inject-on-boot-demo.sh


    install -m 0755 -d ${D}${datadir}/X11/xorg.conf.d
    install -m 0644 ${WORKDIR}/11-evdev-excludes.conf ${D}${datadir}/X11/xorg.conf.d/

    install -m 0755 ${WORKDIR}/renew-on-up.sh ${D}${sysconfdir}/init.d/
}
